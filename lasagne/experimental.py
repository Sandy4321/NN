...
def build_mlp(input_var=None, masks=None, temp=1):
    ...
    l_out = CrossNormalizationLayer(
        l_hid3, nonlinearity=lasagne.nonlinearities.softmax)
    return l_out
...
# ############################# Layers ########################################
class CrossNormalizationLayer(lasagne.layers.Layer):
    def __init__(self, incoming, nonlinearity=lasagne.nonlinearities.softmax,
                 g=None, b=None, **kwargs):
        super(CrossNormalizationLayer, self).__init__(incoming, **kwargs)
    ...
    def get_output_for(self, input, training=True, **kwargs):
        ...
        eps = 1e-3
        norm_input = input - mean.dimshuffle(0,'x')
        norm_input = norm_input/T.sqrt(var + eps).dimshuffle(0,'x')
        output = self.g.dimshuffle('x',0)*norm_input
        output = norm_input
