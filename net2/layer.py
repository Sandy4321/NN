...
class Layer(object):
    ...
    def init_weights(self, nonlinearity):
        
        # Note that in Caffe they call this Xavier
        ...
        elif initialisation_regime == 'ReLU':
            W_shape = self.W.get_value(borrow=True, return_internal_type=True).shape
            b_shape = self.b.get_value(borrow=True, return_internal_type=True).shape
            W_fill  = 0.1*(np.random.randn(W_shape[0], W_shape[1]).astype(dtype=theano.config.floatX))
            b_fill  = 0.1*np.ones(b_shape).astype(dtype=theano.config.floatX)
            self.W.set_value(W_fill)
            self.b.set_value(b_fill)
        else:
    ...
    def get_enc(self, visible):
        ...
        if self.nonlinearity == 'logistic':
            ...
            output = T.maximum(T.dot(visible,self.W) + self.b, 0)
        elif self.nonlinearity == 'softmax':
            output = Tnet.softmax(T.dot(visible,self.W) + self.b)
        else:
