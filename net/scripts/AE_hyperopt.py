...
if __name__ == '__main__':
    ...
    trials = Trials()
    space = (hp.loguniform('fine_tune_learning_rate', np.log(1e-3), np.log(1e1)),
             hp.loguniform('pretrain_learning_rate', np.log(1e-3), np.log(1e1)),
             hp.uniform('momentum', 0.4, 0.99),
             hp.qloguniform('pretrain_epochs', np.log(1), np.log(199), 1),
             hp.qloguniform('tau', np.log(1), np.log(200), 1),
             hp.loguniform('regularisation_weight', np.log(1e-8), np.log(1e-4)),
             hp.loguniform('activation_weight', np.log(1e-8), np.log(1e-4)),
             hp.uniform('h_track', 0.5, 0.99),
             hp.loguniform('sparsity_target', np.log(0.01), np.log(0.5)))
    best = fmin(objective,
        space       = space,
        algo        = tpe.suggest,
        max_evals   = 128, 
